...
3) 串口中断接收：字节写入环形缓冲（USART2/USART3）
...
基于 STM32F767 的工业数据显示系统，使用 LVGL 构建界面，通过串口接收 SQMWD_Tablet 协议数据解析并显示。系统支持 NAND + FatFs 文件系统（字体/资源加载），同时提供 FILE/FRAME 模式切换与调试指令。
